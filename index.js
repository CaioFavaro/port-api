...
// Endpoint único: exibe a localização do usuário
app.get('/api/location', async (req, res) => {
  // Captura o IP do cliente
  let ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
  
  // Se estiver testando localmente (localhost), use um IP de fallback para obter dados reais
  if (ip === '::1' || ip === '127.0.0.1' || (typeof ip === 'string' && ip.startsWith('::ffff:127.0.0.1'))) {
    ip = '8.8.8.8'; // IP público para teste (Google Public DNS)
  }
  
  try {
    // Consulta o serviço ipapi.co para obter os dados de localização
    const response = await axios.get(`https://ipapi.co/${ip}/json/`);
    const locationData = response.data;
    // Cria uma página HTML formatada com os dados de localização
    const html = `
      <!DOCTYPE html>
      <html>
        <head>
